...
const item1 = new Product(1, 'Realme 7', 'Realme', 700, 1500, 70, 'Smartphone');
const item2 = new Product(2, 'Controle de PS5', 'Sony', 100, 200, 200, 'Periféricos');
const item3 = new Product(3, 'Fones de ouvido', 'Genérico', 15, 30, 300, 'Fones');
    ...
    while (menu != 'N' && menu != 'n') {
        ...
        switch (option) {
            ...
            case 3:
                ...
                printProducts()
                const chosenProduct = readLineSync.questionInt('\nQual produto ele deseja comprar? ') -1
                const product = allProducts[chosenProduct]
                console.clear()

                const amout = readLineSync.questionInt('Qual a quantidade? ' )
                console.clear()

                const sell = new Sell(id, client, product, amout, date);

                console.log("Vai ser aplicado desconto nessa venda?\n 1. Sim\n 2. Não")
                const optionDiscount = readLineSync.questionInt()
                console.clear()

                if(optionDiscount === 1) {
                    console.log("O desconto é em:\n 1. Valor fixo\n 2. Porcentagem\n 3. Porcentagem + Valor fixo")
                    const discountMethod = readLineSync.questionInt()
                    console.clear()
                ...
                const chosenMethod = readLineSync.questionInt() - 1;
                const paymentMethodsList: Array<string> = ['PIX','Crédito','Débito','Dinheiro']
                const paymentMethod = paymentMethodsList[chosenMethod]
                console.clear()
